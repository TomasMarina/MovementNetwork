...
vertex.attributes(net_col_pospen)
length(V(net_col_pospen)$lobos == "1")
sum(degree(net_colonia, v = V(net_colonia)$lobos == "1", mode = "total"))
hist(degree(net_col_prepen, v = V(net_col_prepen)$lobos == "1", mode = "total"))

degree.distribution(net_col_pospen, v = V(net_col_pospen)$lobos == "1", mode = "total")
...
netmap_col_lobos_con <- ggplot() +
  ...
  geom_segment(data = links_plot_col_pospen,
               aes(x = x, xend = xend, y = y, yend = yend),
               arrow = arrow(length = unit(0.1, "inches"), type = "closed"),
               colour = ifelse(links_plot_col_pospen$lobos == 1, "red", "blue"),
               size = 0.5, alpha = 0.5) +
...
hist_cam <- ggplot(caminos, aes(x = Frecuencia)) +
  geom_bar() +
  labs(x = "Caminos más cortos colonia-lobos", y = "Frecuencia")
